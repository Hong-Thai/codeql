...
class NodeJSCryptoUsage extends CryptoAPIUsage {
    ...
    NodeJSCryptoUsage(){
        exists(DataFlow::SourceNode mod |
            mod = DataFlow::moduleImport("crypto") 
        ...
        exists(DataFlow::SourceNode mod |
            mod = DataFlow::moduleImport("crypto") 
            and call = mod.getAPropertyRead("webcrypto").getAPropertyRead("subtle").getAMethodCall()
            |
            this = call
            and location = call.asExpr().getLocation() 
            and numArgs = call.getNumArgument() 
    ...
    override string getCryptoAPIName() { result = "NodeJsCrypto" }
